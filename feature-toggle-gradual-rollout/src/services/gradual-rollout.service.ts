...
export class GradualRolloutService {
  ...
  constructor() {
    ...
    setInterval(
      ...
      parseInt(process.env.ROLLOUT_UPDATE_INTERVAL || '60000')
    ) // 1 minute
  }
  ...
  async isFeatureEnabled(
    ...
    try {
      ...
      const existingAssignment = await this.featureFlagRepo.getUserFeatureAssignment(
        user.id,
        featureFlag.id
      )
  ...
  private async recordAnalytics(
    ...
    if (process.env.ENABLE_FEATURE_ANALYTICS === 'true') {
      try {
        await this.featureFlagRepo.recordFeatureAnalytics(
          featureFlagId,
          userId,
          eventType,
          eventData
