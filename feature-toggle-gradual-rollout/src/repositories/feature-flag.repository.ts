...
export class FeatureFlagRepository {
  ...
  async updateRolloutPercentage(
    ...
    try {
      ...
      const currentResult = await client.query('SELECT * FROM feature_flags WHERE name = $1', [
        name,
      ])
  ...
  async getUserFeatureAssignment(
    userId: string,
    featureFlagId: number
  ): Promise<UserFeatureAssignment | null> {
    const query = `
